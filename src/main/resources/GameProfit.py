...
def gameProfitETL(path, pathP, dest):
    ...
    try:
        ...
        dataframeOuptut = pd.DataFrame(
            columns=['EGM', 'Game', 'Total In', 'Total Out', 'Games Played', 'Games Won', 'Date'])
        for file in csvfiles:
            ...
            dataset.loc[:, ['Total In', 'Total Out', 'Games Played', 'Games Won']] = dataset.loc[:,
                                                                                     ['Total In', 'Total Out',
                                                                                      'Games Played',
                                                                                      'Games Won']].replace(
                to_replace="[%]", value="", regex=True).replace(to_replace='[nan]', value='0', regex=True)
            try:
